...
describe("registro azioni", () => {
  ...
  test("le sedici azioni approvate esistono, non una di più e non una diversa", () => {
    // 16/09/2026 — sedici, non piu' quindici: cancella_documento e' la nuova azione, nata per dare
    // a "cancella X" un posto vero quando X e' un documento del percorso (non un evento in agenda).
    assert.equal(
      app.AZIONI_CONVERSAZIONALI.map((a) => a.id).join(","),
      "apri_percorso,crea_percorso,salva_nel_percorso,apri_documento,cancella_documento,scrivi_su_pilastro,crea_seme,interroga_memoria,avanza_percorso,chiudi_percorso,crea_evento_calendario,sposta_evento_calendario,invia_mail,leggi_calendario,trova_evento_calendario,cancella_evento_calendario"
    );
  });
