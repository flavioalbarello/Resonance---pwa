...
describe("registro azioni", () => {
  ...
  test("le diciassette azioni approvate esistono, non una di più e non una diversa", () => {
    // 16/09/2026 (sera) — diciassette, non piu' sedici: modifica_documento e' la nuova azione, nata
    // perche' l'unica via per aggiungere una parola a un documento era "cancella tutto e riscrivi".
    assert.equal(
      app.AZIONI_CONVERSAZIONALI.map((a) => a.id).join(","),
      "apri_percorso,crea_percorso,salva_nel_percorso,apri_documento,cancella_documento,modifica_documento,scrivi_su_pilastro,crea_seme,interroga_memoria,avanza_percorso,chiudi_percorso,crea_evento_calendario,sposta_evento_calendario,invia_mail,leggi_calendario,trova_evento_calendario,cancella_evento_calendario"
    );
  });
