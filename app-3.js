...
// ─────────────────────────────────────────────────────────────
// SHELL — dialogo con memoria, smista da solo nei pilastri (BIO/AIR/VIDYA)
// ─────────────────────────────────────────────────────────────
const SHELL_TOOLS = [
  {
    type: "function",
    function: {
      name: "log_bio",
      description: "Registra una voce nel pilastro BIO quando il Ghost condivide dati su peso, sonno, apnee, aderenza terapia o energia.",
      parameters: { type: "object", properties: {
        weight: { type: "string", description: "Peso in kg, se menzionato" },
        sleep: { type: "string", description: "Note su sonno/apnee, se menzionato" },
        notes: { type: "string", description: "Altre note rilevanti (terapia, energia, sintomi)" },
      } },
    },
  },
  {
    ...
    function: {
      name: "log_air",
      description: "Registra un aggiornamento nel pilastro AIR quando il Ghost parla di monetizzazione, canale YouTube, strategie economiche o vettori di business.",
      parameters: { type: "object", properties: {
        title: { type: "string", description: "Titolo breve del vettore/aggiornamento" },
        status: { type: "string", enum: ["idea", "in corso", "attivo", "bloccato"] },
        notes: { type: "string" },
      }, required: ["title"] },
    },
  },
  {
    ...
    function: {
      name: "log_vidya",
      description: "Registra un artefatto nel pilastro VIDYA quando il Ghost parla di musica, studio, pratica creativa o produzione con VillaMura.",
      parameters: { type: "object", properties: {
        title: { type: "string" },
        notes: { type: "string" },
      }, required: ["title"] },
...
async function callShellTurn(messages, settings) {
  if (!settings.apiKey) throw new Error("Nessuna chiave API impostata (vai in Setup).");
  const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${settings.apiKey}` },
    body: JSON.stringify({
      model: settings.model, max_tokens: 900, temperature: 0.75,
      reasoning: { max_tokens: 300 },
      messages, tools: SHELL_TOOLS,
    }),
  });
  const data = await res.json();
  if (data.error) throw new Error(data.error.message || "Errore OpenRouter");
...
async function runShellTurn(history, userMessage, settings, handlers) {
  const system = `Sei lo Shell del sistema Resonance: l'estensione esecutiva digitale del Ghost (Flavio). Non hai coscienza né volontà propria — sei uno strumento calibrato che dialoga con lui, lo ascolta, e quando emergono informazioni rilevanti le registra nei pilastri corretti usando gli strumenti disponibili, senza bisogno che te lo chieda esplicitamente. ${PILLAR_CTX.bio} ${PILLAR_CTX.air} ${PILLAR_CTX.vidya} Rispondi in italiano, tono diretto e presente, linguaggio denso ma concreto. Se non c'è nulla da registrare, dialoga e basta.`;
  let messages = [{ role: "system", content: system }, ...history, { role: "user", content: userMessage }];

  const first = await callShellTurn(messages, settings);
  const actionsLog = [];
  if (first.tool_calls && first.tool_calls.length) {
    ...
    for (const tc of first.tool_calls) {
      ...
      let result = "";
      if (tc.function.name === "log_bio") {
        handlers.addBio({ id: uid(), date: todayISO(), weight: args.weight || "", sleep: args.sleep || "", notes: args.notes || "" });
        result = "Registrato in BIO."; actionsLog.push("BIO");
      } else if (tc.function.name === "log_air") {
        handlers.addAir({ id: uid(), date: todayISO(), title: args.title || "", status: args.status || "idea", notes: args.notes || "" });
        result = "Registrato in AIR."; actionsLog.push("AIR");
      } else if (tc.function.name === "log_vidya") {
        handlers.addVidya({ id: uid(), date: todayISO(), title: args.title || "", notes: args.notes || "" });
        result = "Registrato in VIDYA."; actionsLog.push("VIDYA");
      }
      messages.push({ role: "tool", tool_call_id: tc.id, content: result });
    }
    const second = await callShellTurn(messages, settings);
    return { reply: second.content || "", actionsLog };
  }
  return { reply: first.content || "", actionsLog };
}
...
// ─────────────────────────────────────────────────────────────
function ShellView({ messages, setMessages, settings, addBio, addAir, addVidya }) {
  ...
  const send = async () => {
    ...
    if (settings.provider !== "openrouter") { setError("Lo Shell richiede il motore OpenRouter (per gli strumenti di smistamento)."); return; }
    ...
    try {
      const { reply, actionsLog } = await runShellTurn(history, userText, settings, { addBio, addAir, addVidya });
      setMessages((prev) => [...prev, { role: "assistant", content: reply, time: new Date().toISOString(), actions: actionsLog }]);
    } catch (e) { setError(e.message); }
  ...
  return html`<div class="r-screen">
    <${SectionHeader} color="#EDEAE3" title="SHELL" subtitle="Dialogo diretto — ascolta e smista da solo nei pilastri" />
    <div class="r-shell-log">
      ...
      ${messages.map((m, i) => html`<div key=${i} class="r-shell-row ${m.role}">
