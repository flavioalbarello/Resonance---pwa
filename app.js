...
const APP_BUILD = "2026-07-18 · artifacts-crosspillar-v1";
...
function PercorsoDetail({ pillar, color, percorso, onUpdate, onBack, onDelete, settings }) {
  ...
  const generateArtifact = async () => {
    ...
    try {
      const sys = `Sei lo Shell del sistema Resonance, pilastro ${pillar.toUpperCase()}. Genera un documento strutturato e concreto in italiano, basato sulla richiesta del Ghost, coerente col percorso "${percorso.title}". Usa questo markup leggero: "# " per il titolo principale, "## " per le sezioni, "- " per gli elenchi, righe normali per i paragrafi. Niente fronzoli, niente premesse: solo il documento.`;
      const text = await askModel(sys, artBrief.trim(), 0.6, 1800, settings);
      setArtText(text);
