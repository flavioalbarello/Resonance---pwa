...
async function runTriadeMagi(question, onStage, settings, opts = {}) {
  ...
  const balthasar = await askModel(`${baseCtx}${memoriaCtx}${targetCtx} Sei BALTHASAR, il Perturbatore. Genera una divergenza evolutiva su questo tema, audace, non convenzionale — a intensità "${intensity}" (leggera = uno spostamento laterale; profonda = una rottura vera con l'assetto attuale).`, question, balthasarTemp, 1600, settings);
  ...
  const caspar = await askModel(`${baseCtx} Sei CASPAR, l'Ancora. Verifica il piano contro i vincoli assoluti: salute, tempo lineare del Ghost, sostenibilità economica, compartimentazione identità professionale (PhysioAlba mai esposta). ${containmentCtx}\n\nPiano: "${melchior}"`, question, 0.2, 1600, settings);
...
function buildResonanceDigest({ bio, air, vidya, kernel, magi, pBio, pAir, pVidya }) {
  ...
  const pctx = (list) => list.length ? list.map((p) => `"${p.title}"${p.kind === "identitario" ? " [già identitario]" : ""}`).join(", ") : "nessuno";
...
async function computeResonance(digest, settings) {
  const data = await askModelJSON(
    `Sei la funzione SIMBIOSI del sistema Resonance: non un pilastro operativo, ma il punto di incontro tra BIO, AIR, VIDYA e il Kernel. Hai tre mandati (Manifesto V3 §5):
...
VINCOLO ASSOLUTO: non suggerire MAI di integrare/esporre/collegare l'identità professionale del Ghost (fisioterapista, PhysioAlba) con AIR o altro — compartimentazione voluta e permanente, non una discrepanza da risolvere.
Rispondi SOLO con JSON:
{
  "text": "3 parti separate da riga vuota: 1) giudizio qualitativo breve (mai un numero); 2) posizionamento tra ordine e caos + discrepanze specifiche; 3) una singola azione concreta — se emerge cristallizzazione, può essere proporre di portare un tema preciso in Agorà Magi (quale + intensità leggera/profonda)",
  ...
  if (!data) return { text: "Valutazione non riuscita (risposta non interpretabile). Riprova.", worthSurfacing: false, identityHint: null };
  return { text: data.text || "", worthSurfacing: !!data.worthSurfacing, identityHint: data.identityHint || null };
}
      ...
      ${percorsi.length === 0 ? html`<${Empty} text="Nessun percorso ancora." />` : html`
        <div class="r-list">${percorsi.map((p) => {
          ...
          return html`<${Card} accent=${color}><div class="r-entry-row" style="cursor:pointer" onClick=${() => setSelectedId(p.id)}>
            <div><div class="r-entry-line"><b>${p.title}</b>${p.kind === "identitario" ? html` <span class="r-badge" style="border-color:${color};color:${color}">identitario</span>` : ""}</div>
...
function PercorsoDetail({ pillar, color, percorso, onUpdate, onBack, onDelete, settings }) {
  ...
  return html`
    <div>
      ...
      <${Card} accent=${color}>
        <div class="r-hub-title" style="color:${color}">${percorso.title}${percorso.kind === "identitario" ? html` <span class="r-badge" style="border-color:${color};color:${color}">identitario</span>` : ""}</div>
...
function Hub({ bio, air, vidya, magi, resonance, setView, pBio, pAir, pVidya, proactiveHint }) {
  ...
  return html`<div class="r-screen">
    ...
    <div class="r-hub-grid">
      ...
      <${Card} accent="#D9B872"><div class="r-hub-row" onClick=${() => setView("simbiosi")}><div><div class="r-hub-title">SIMBIOSI${proactiveHint ? html` <span style="color:${C.core}">●</span>` : ""}</div>
...
function App() {
  ...
  const recalcResonance = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const res = await computeResonance(digest, settingsRef.current);
  ...
  const promoteToIdentity = useCallback((hint) => {
    if (!hint?.pillar || !hint?.title) return;
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      resonanceBusyRef.current = true;
      saveKey("simbiosi-eval-signature", signature);
      try {
      ...
      } catch (e) { pushDebugLog({ type: "simbiosi-proactive", error: e.message }); }
