...
const APP_BUILD = "2026-07-20 · onboarding-v2-rich-profile";
...
function buildPillarCtx(profile) {
  ...
  return {
    vidya: cogText,
    bio: bioList.join("; ") + " Ogni lettura BIO è una stance interpretativa rivedibile dal Ghost, mai un verdetto medico oggettivo.",
    air,
  };
}
...
function OnboardingView({ onComplete, settings }) {
  ...
  const [channel, setChannel] = useState("uditivo");
  ...
  const finalize = () => {
    ...
    onComplete({
      name: name.trim(),
      hardConstraints: hc,
      cognitiveStyle: { channel, density, dialectic, dialecticOverride: null, reasoningStyle },
      freeform: {
  ...
  return html`<div class="r-screen">
    <div class="r-section-header"><h2>Benvenuto in Resonance</h2>
      <p>Quello che scrivi qui viene salvato sul tuo Google Drive personale — nessun altro lo vede. Viene usato solo da Resonance per calibrarsi su di te. Puoi modificarlo o cancellarlo in qualsiasi momento. Nessuna domanda è obbligatoria: se non vuoi rispondere ora, usa il pulsante dedicato. Questo strumento non sostituisce un medico o uno psicologo.</p></div>
    ...
    <div class="r-card">
      ...
      <label class="r-field"><span>Come ti arriva meglio? Quando impari qualcosa di nuovo, preferisci uno schema, che te lo raccontino a voce, o provarlo con le mani?</span>
        <select class="r-input" value=${channel} onChange=${(e) => setChannel(e.target.value)}>
          <option value="visivo">Visivo</option><option value="uditivo">Uditivo</option><option value="pratico">Pratico</option>
        </select></label>
      <label class="r-field"><span>Quanta carne sul fuoco — risposte corte e dirette, o lunghe e complete?</span>
        <select class="r-input" value=${density} onChange=${(e) => setDensity(e.target.value)}>
          <option value="semplice">Semplice</option><option value="densa">Densa</option>
        </select></label>
      <div class="r-settings-row" style="margin-bottom:10px">
        <span>Confronto o conferma? Preferisci essere messo alla prova o rassicurato? (potrai cambiarlo in ogni conversazione)</span>
        <input type="checkbox" checked=${dialectic} onInput=${(e) => setDialectic(e.target.checked)} />
      </div>
      <label class="r-field"><span>Il tuo modo di ragionare — liste e passi, o salti tra idee e intuizioni?</span>
        <select class="r-input" value=${reasoningStyle} onChange=${(e) => setReasoningStyle(e.target.value)}>
          <option value="strutturato">Strutturato</option><option value="saltellante">Saltellante</option><option value="misto">Un misto</option>
        </select></label>
    </div>
    <div class="r-card">
      <h3>Cosa non si tocca</h3>
      <label class="r-field"><span>Vincoli assoluti — cosa non va mai toccato o suggerito, nemmeno come battuta? Una voce per riga; se puoi aggiungi il perché (es. "zucchine — intolleranza").</span>
      ...
      <label class="r-field"><span>La regola d'oro — tra tutto quello sopra, la più importante?</span><input class="r-input" value=${priority} onInput=${(e) => setPriority(e.target.value)} /></label>
    </div>
    <div class="r-card">
      <h3>Salute <small style="opacity:.7">(puoi saltare qualunque punto qui)</small></h3>
      <${SkippableField} label="Farmaci e terapie in corso — anche il dosaggio se ti va, aiuta a evitare conflitti evidenti." value=${medical} onChange=${setMedical} placeholder="farmaco/terapia + dosaggio se noto" textarea=${true} />
    </div>
    <div class="r-card">
      <h3>Il resto</h3>
      <${SkippableField} label="Il vuoto che nessuno ha riempito — cosa vorresti che questo sistema facesse per te?" value=${motivation} onChange=${setMotivation} textarea=${true} />
      <${SkippableField} label="Il tuo contesto — qualcosa di utile per capire chi sei ora?" value=${context} onChange=${setContext} textarea=${true} />
      <${SkippableField} label="Una cosa sola — se potesse fare perfettamente una cosa sola per te?" value=${request} onChange=${setRequest} />
