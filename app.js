...
async function askModelJSON(system, userText, temperature, maxTokens, settings, image = null) {
  ...
  if (!raw) return null;
  const cleaned = stripTrailingCommas(raw.replace(/```json|```/g, "").trim());
  ...
  if (block) {
    try { return JSON.parse(stripTrailingCommas(block)); } catch { /* prova comunque a loggare sotto */ }
  }
...
Rispondi SOLO con JSON:
{
  ...
  return { text: data.text || "", worthSurfacing: !!data.worthSurfacing, identityHint };
}
