...
const CAPACITA = [
  ...
  { n: `Banco microfono in auto`, k: ["microfono", "auto", "macchina", "prova-voce"], s: `Banco microfono in auto: si apre dal pulsante BANCO MICROFONO nella schermata principale (Hub), in fondo alla griglia. E' una pagina separata dall'app — misura il BROWSER, quindi deve funzionare anche se l'app non si disegnasse — e ha un pulsante «Torna a Resonance» in cima. Misura quale microfono usa davvero il browser quando il telefono e' collegato all'auto, quanto capisce il riconoscimento vocale in tre condizioni (telefono in mano, in tasca, in tasca col finestrino aperto), e da dove esce l'audio. Si esegue DA FERMI, motore acceso, perche' la pagina va guardata. Alla fine c'e' un registro da copiare. Non tocca nessun dato: e' un banco di misura, non una funzione. Fino al 14/09/2026 ci si arrivava solo digitando l'indirizzo a mano, e per questo non era mai stata usata.` },
...
function Hub({ bio, air, vidya, magi, resonance, setView, pBio, pAir, pVidya, proactiveHint, postura }) {
  ...
  return html`<div class="r-screen">
    ...
    <div class="r-hub-grid">
      ...
      ${/* 14/09/2026 — IL BANCO MICROFONO SI APRE DA QUI, e il motivo è un difetto mio.
            La pagina esisteva dall'11/09 e per usarla bisognava digitare un indirizzo: il Ghost ha
            provato, ha sbagliato dominio (gli avevo dato un segnaposto come se fosse un indirizzo) e
            si è preso un 404. «Troppo macchinoso» — e aveva ragione due volte: una prova che per
            partire chiede di dettare un URL non verrà mai eseguita.
            Il collegamento è RELATIVO di proposito: così l'indirizzo giusto è sempre quello da cui
            l'app è già aperta, e non c'è niente da sapere né da scrivere. Resta una pagina a sé e
            non una schermata dell'app, perché misura il BROWSER: se un giorno l'app non si
            disegnasse, il banco deve funzionare lo stesso. */ ""}
      <${Card} accent=${C.core}><a class="r-hub-row" href="./prova-voce.html" style="text-decoration:none;color:inherit;display:block"><div><div class="r-hub-title" style="color:${C.core}">🎤 BANCO MICROFONO</div>
        <div class="r-hub-detail">Da fermo, in macchina: misura quale microfono usa il telefono, quanto capisce e da dove esce la voce. Non tocca nessun dato.</div></div></a></${Card}>
    </div>
  </div>`;
}
