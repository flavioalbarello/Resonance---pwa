...
const APP_BUILD = "2026-07-19 · calendar-fused-toggle-v1";
...
Se noti un argomento di studio/lavoro strutturato e continuativo emergere (non un dato isolato), PROPONI a parole di aprire un percorso dedicato ("Vuoi che apra un percorso su questo?"). Non crearlo tu.`;
  ...
  // Risposta, lettura multi-lente (+ Calendar fuso, se abilitato) e bozza: indipendenti, partono insieme
  ...
  return { reply, actionsLog, anochin, proposal, alerts, newStyleMemory, draft, calendarProposal };
}
...
function ShellView({ messages, setMessages, settings, addBio, addAir, addVidya, percorsi, setPercorsi, memory, updateMemoria, styleMemory, setStyleMemory, bio, air, vidya, pushDebugLog }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      const { reply, actionsLog, anochin, proposal, alerts, newStyleMemory, draft, calendarProposal } = await runShellTurn(history, userText, settings, { addBio, addAir, addVidya, updateMemoria }, memory, styleMemory, currentAttachment);
      setMessages((prev) => [...prev, { id: assistantMsgId, role: "assistant", content: reply, time: new Date().toISOString(), actions: actionsLog, anochin, proposal, alerts, draft, calendarProposal }]);
