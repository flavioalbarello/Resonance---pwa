...
async function askOpenRouter(system, userText, temperature, maxTokens, apiKey, model, useWebSearch) {
  const body = {
    ...
    reasoning: { effort: "low" }, // riduce il "pensiero" interno (es. Gemini 3.x) per lasciare spazio alla risposta visibile
  };
...
async function runAirAgent(task, settings) {
  ...
  return askOpenRouter(system, task, 0.7, 1300, settings.apiKey, settings.model, true);
}
...
async function decomposeTopics(pillar, title, settings) {
  const data = await askModelJSON(
    ...
    `Percorso: "${title}"`, 0.6, 700, settings
  );
  return data?.topics || [];
}
