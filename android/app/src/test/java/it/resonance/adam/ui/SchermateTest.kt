...
class SchermateTest {
    ...
    private fun semina(vm: Adam) = runBlocking {
        ...
        db.taccuino().inserisci(it.resonance.adam.dati.Nota(testo = "Un planner per chi suona in una tribute band: nicchia scoperta?", creata = System.currentTimeMillis(), ripresa = System.currentTimeMillis()))
        db.fondo().inserisci(it.resonance.adam.dati.Movimento(giorno = g(2), tipo = it.resonance.adam.dati.TipoMovimento.VERSAMENTO, importo = 100.0, motivo = "Primo versamento", creato = 1))
        db.fondo().inserisci(it.resonance.adam.dati.Movimento(giorno = g(1), tipo = it.resonance.adam.dati.TipoMovimento.USCITA, importo = 12.0, motivo = "Dominio neutro, un anno", creato = 2))
        db.lettere().inserisci(it.resonance.adam.dati.Lettera(oggetto = "Fondo: primo mese", testo = "Contesto… Domande chiuse…", creata = 1, stato = it.resonance.adam.dati.StatoLettera.INVIATA, numero = 1))
