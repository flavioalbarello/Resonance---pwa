...
**Data**: 10/09/2026 · **Build**: `2026-09-10 · non-esiste-non-e-piu-una-risposta` · **Service worker**: `resonance-v16`
**Rami**: `main` (Flavio/Ghost) e `stable` (Marta), allineati — `git diff origin/main origin/stable` vuoto.
**Prove**: 601 verdi, 117 suite, 26 file.
...
> **V+1, non sovrascrittura** (Legge 14). Questa è la seconda istanza del rapporto.
> `RAPPORTO_STATO_2026-09-07.md` resta com'era: è la fotografia di quel giorno, e conteneva due
> errori miei — dieci azioni conversazionali invece di quindici, e un elenco di prove poi cresciuto.
> Correggerlo in luogo avrebbe cancellato la prova che quegli errori erano stati fatti.
...
```
app.js                 11.028 righe · monolite, tutto il resto è supporto
...
tests/                          26 file di prova + 3 di supporto
```
...
**Le QUINDICI azioni conversazionali** — elenco chiuso, il modello sceglie, il programma esegue:
...
> Questo elenco diceva **dieci** fino al 10/09/2026, ed era un errore mio: le avevo contate su una
> finestra di testo troncata. Due delle cinque mancanti — `invia_mail` e `cancella_evento_calendario`
> — hanno **effetto esterno**. Ora il numero è pintato da una prova (`identita-e-recinto`), quindi
> non può più driftare in silenzio.
...
| **`smentisciAssenzaDiMateriale`** | **il modello che dichiara ASSENTE del materiale che c'è** | **10/09** |

### Il nono filtro, e perché gli altri otto non bastavano

I primi otto cercano tutti **la stessa cosa**: il modello che dichiara *compiuto* ciò che non è
avvenuto. Nessuno guardava il verso opposto — **il sistema era blindato sul falso positivo e
spalancato sul falso negativo**.

Costato un documento dato per perso: alla domanda «dimmi i temi dell'Atto IV e dell'Atto V» lo Shell
ha risposto «non esistono», due volte, mentre erano nel percorso da otto giorni. Dettaglio completo
in `REPORT_NON_ESISTE_2026-09-10.md`.

Il falso negativo è **peggio**: un «l'ho messo in calendario» falso si scopre aprendo il calendario;
un «non esiste» falso è la risposta che **chiude la ricerca**, e non dà a chi legge nessun motivo di
verificare.

`smentisciAssenzaDiMateriale` è il gemello di `smentisciCapacitaSpenta`: se il modello nega e il
programma ha in mano l'elenco, la frase è falsa su un dato verificabile. **Non cancella, aggiunge**
la smentita coi titoli veri; **senza dato tace**; **due criteri** (deve negare *e* parlare di
materiale), o «non ci sono controindicazioni» diventerebbe una smentita.
...
**601 prove, 117 suite, 26 file.** Nessuna dipendenza esterna: solo `node:test`.

```
anello · balthasar · calendario · capitolato · degenerazione · documenti-nel-contesto
finestra-conversazione · generatore · griglia
identita-e-recinto · magi-forma · meta-narrazione · negazioni-e-ricerca
...
| **Due correzioni non difese nel punto d'innesto** | `runSeedGateCheck` (l'ordine gate→effettore) e `smentisciAssenzaDiMateriale` (la riga che lo invoca nella catena del turno). Entrambe sono **proprietà d'ordine**: staccandole, il banco resta verde. I binding di un modulo ES non sono sostituibili dall'esterno — servirebbe iniezione delle dipendenze, schema già in uso due volte qui. Verificato per esperimento, non dedotto. |
| **La regola dell'inventario insegna a negare** | «se un percorso è dichiarato con NESSUN documento salvato […] **dillo**» non distingue «il percorso è vuoto» da «non ho ricevuto quel documento». Il nono filtro la compensa a valle; la frase è ancora nel prompt. |
| **`TETTO_DOCUMENTO_IN_RICERCA = 600`** | Un documento di 4269 caratteri arriva al **14%**. Per «dimmi i temi» basta, per lavorarci no. |
