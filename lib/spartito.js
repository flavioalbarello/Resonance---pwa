...
export function spiegazioneRicerca({ query, strumento = "", esito, archivio = ARCHIVIO_SPARTITI }) {
  ...
  const nota = strumento
    ...
    : "";
  if (e.errore) {
    return { caso: "errore", testo: `Ho provato a cercare «${query}» in ${archivio.nome} e non ho ottenuto risposta: ${e.errore}. Non è una ricerca a vuoto, è una ricerca che non è partita: riprova fra poco.${nota}` };
  }
  const coda = `Quell'archivio tiene ${archivio.perChe}. Rock, metal, pop: lì non ci sono e non ci saranno. Non è un guasto, è il suo repertorio — e non ne ho altri: gli archivi di tablature per quella musica non si lasciano interrogare da un'app come questa.\n\n`
    + `Cosa posso fare davvero: cercare con un altro titolo (la casella qui sotto), oppure scriverti una linea io dal riquadro "Spartito" dentro un percorso — ma quella la inventa un modello, non è la trascrizione di quel brano, e te lo dico prima perché la differenza conta.${nota}`;
  if (!tunes.length && !grezzi) {
    return { caso: "nessuno", testo: `Ho cercato «${query}» in ${archivio.nome} — è l'unico archivio che so interrogare — e non conosce nessun brano con queste parole. Ha risposto zero, e zero è zero: non c'era niente da scegliere.\n\n${coda}` };
  }
  if (!tunes.length) {
    ...
    return { caso: "potati", testo: `Ho cercato «${query}» in ${archivio.nome} — è l'unico archivio che so interrogare — e non c'è.${perche}\n\n${coda}` };
  }
  ...
  return { caso: "trovati", testo: `Ho cercato «${query}» in ${archivio.nome}: ${pertinenti} ${pertinenti === 1 ? "brano" : "brani"}`
