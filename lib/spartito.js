...
// Aveva ragione, e non è un giudizio di gusto: era letteralmente la scala, salita e ridiscesa.
//
// È il modo tipico in cui un modello finge di aver capito una richiesta musicale: gli si nomina una
// tonalità esotica, lui non sa cosa farne, e restituisce il MATERIALE invece della MUSICA — le note
// che quella tonalità contiene, in fila. Ha la forma giusta (metro, battute che tornano, tonalità
// dichiarata) e supera ogni requisito che guardi la forma. È la stessa famiglia del «modello dice a
// parole»: qui dice di aver scritto una linea di basso, e il programma deve andare a guardare.
//
// COSA DISTINGUE UNA SCALA DA UNA LINEA: una linea ha una figura che torna, salti verso le note
// dell'accordo, un respiro ritmico. Una scala va per gradi congiunti nella stessa direzione per
// tutta la sua lunghezza. Non serve giudicare la bellezza — serve accorgersi dei gradi congiunti.
const GRADO_DI = { C: 0, D: 1, E: 2, F: 3, G: 4, A: 5, B: 6 };
// L'altezza in GRADI di scala (non in semitoni): le alterazioni non contano, perché una scala è
// tale in qualunque modo — e in do locrio o in re maggiore i gradi congiunti sono sempre gradi.
export function gradiDelleNote(testo) {
  const fuori = [];
  for (const n of noteDi(testo)) {
    const m = /([A-Ga-g])([,']*)/.exec(n);
    if (!m) continue;                                  // pause e accordi: non sono un'altezza singola
    const lettera = m[1];
    const base = GRADO_DI[lettera.toUpperCase()];
    let ottava = lettera === lettera.toLowerCase() ? 7 : 0;
    for (const c of m[2]) ottava += c === "'" ? 7 : -7;
    fuori.push(base + ottava);
  }
  return fuori;
}
// La corsa più lunga per gradi congiunti nella stessa direzione, e quanta parte del brano occupa.
// Si guarda la corsa PIÙ LUNGA e non la media: una linea vera può avere una scaletta di passaggio
// di quattro note senza per questo essere una scala.
export function corsaDiScala(testo) {
  const g = gradiDelleNote(testo);
  if (g.length < 8) return { corsa: 0, note: g.length, quota: 0 };
  let corsa = 1, massima = 1, direzione = 0;
  for (let i = 1; i < g.length; i++) {
    const salto = g[i] - g[i - 1];
    const verso = Math.sign(salto);
    if (Math.abs(salto) === 1 && (direzione === 0 || verso === direzione)) { corsa++; direzione = verso; }
    else if (Math.abs(salto) === 1) { corsa = 2; direzione = verso; }
    else { corsa = 1; direzione = 0; }
    if (corsa > massima) massima = corsa;
  }
  return { corsa: massima, note: g.length, quota: massima / g.length };
}
...
export function battuteCheNonTornano(abc, battute, metro, unita) {
  ...
  // UN'ANACRUSI E' UN'ECCEZIONE, NON LA NORMA — 15/09/2026, dal caso peggiore della sessione.
  // Questa regola accoppiava QUALSIASI coppia di parziali adiacenti. Su uno spartito di 4 battute
  // che valevano 4 crome ciascuna invece di 8, le coppie tornavano — 4+4=8, 4+4=8 — e TUTTE le
  // battute sbagliate passavano senza una parola. Era la levata iniziale che si accoppia con la
  // battuta finale, generalizzata troppo: applicata a tappeto, assolve uno spartito interamente
  // sbagliato purché lo sia in modo regolare. Che è esattamente come sbaglia un modello.
  // Una levata riguarda le battute di BORDO, quindi le parziali sono una minoranza. Se sono la
  // maggioranza non è una levata: è una trascrizione che non torna.
  if (parziali.length * 2 > (battute || []).length) {
    return `${parziali.length} battute su ${(battute || []).length} non valgono ${attesa} unità di L:: non è una levata, è uno spartito che non torna`;
  }
...
export const REQUISITI_SPARTITO = [
  ...
  {
    // 15/09/2026 — IL PEGGIO CHE QUESTO PROGRAMMA ABBIA FATTO. Il modello, davanti a un PDF che non
    // riusciva a leggere, si è comportato BENE: l'ha detto, dentro i commenti dell'ABC.
    //   % Non riesco a vedere l'immagine dello spartito. Il file fornito è un PDF senza contenuto
    //   % leggibile. Non posso inventare note: scrivo pause per tutta la durata prevista.
    // E il programma gliel'ha spacciata per uno spartito trovato, disegnata sul pentagramma, col
    // pulsante «Tieni». Il modello ha detto la verità e il programma non è andato a guardare: è la
    // regola di casa — «il modello dice a parole, il programma va a cercarlo davvero» — rotta da
    // chi la scrive. Una rinuncia dichiarata è la cosa PIU' facile da verificare che ci sia, e non
    // la verificava nessuno.
    id: "niente-rinuncia-dichiarata",
    detta: "Se non riesci a leggere il documento dillo in chiaro e NON produrre ABC: una pagina di pause non è una trascrizione. Meglio niente che una cosa che sembra musica.",
    verifica: (a) => {
      const rinuncia = /(?:non\s+(?:riesco|posso|sono\s+in\s+grado)|impossibile\s+(?:leggere|vedere)|senza\s+contenuto\s+leggibile|unable\s+to|cannot\s+(?:read|see)|no\s+readable)/i.exec(a.commenti || "");
      return rinuncia ? `il modello ha dichiarato di non essere riuscito a leggere («${rinuncia[0]}»): non è uno spartito, è una rinuncia` : null;
    },
  },
  {
    // E la sua conseguenza materiale: quello che ne era uscito erano quattro battute di SOLE PAUSE.
    // Il silenzio non è musica, e uno spartito senza una nota non è uno spartito — qualunque sia il
    // motivo per cui è venuto così.
    id: "almeno-una-nota",
    detta: "Uno spartito deve contenere NOTE. Un corpo di sole pause (z) non è una trascrizione.",
    verifica: (a) => /[A-Ga-g]/.test(String(a.corpo || "").replace(/"[^"]*"/g, " ").replace(/![^!]*!/g, " "))
      ? null : "nel corpo non c'è nemmeno una nota: sono tutte pause",
  },
  {
  ...
  },
  {
    ...
    // «archivio» = una trascrizione scritta DA UNA PERSONA e copiata da una pagina. Su 192 vere,
    // 31 hanno battute che non tornano ed è musica vera scritta a orecchio: per loro è un avviso.
    // «lettura» = un MODELLO che ha guardato un PDF o un'immagine. NON è la stessa cosa, e averle
    // confuse è costato caro il 15/09: la tolleranza pensata per chi conosce il brano e sbaglia a
    // contare le crome veniva regalata a chi il brano non l'ha mai sentito e sta tirando a indovinare.
    // Un umano sbaglia il conteggio; un modello inventa. Per la lettura di un documento le battute
    // che non tornano sono il segnale PIU' forte che non ha letto niente: è un errore.
    avvisoPer: ["archivio"],
...
export function analizzaSpartito(abc, origine = "modello", extra = []) {
  ...
  const a = {
    ...
    // 15/09/2026 — I COMMENTI SI RACCOLGONO INVECE DI BUTTARLI. Fino a oggi la riga qui sotto era
    // `if (eCommento(riga) || !riga.trim()) return;`: i commenti uscivano PRIMA di qualunque
    // controllo, quindi nessun controllo poteva vederli. Ed è dentro un commento che il modello ha
    // scritto «Non riesco a vedere l'immagine dello spartito»: la cosa più importante che avesse da
    // dire, nell'unico posto dove il programma non guardava.
    // E il CORPO, per poter chiedere se contiene almeno una nota o soltanto pause.
    commenti: "", corpo: "",
  };
  let vistoK = false;
  let noteDellUltimaRiga = 0;
  righe.forEach((riga, i) => {
    if (eCommento(riga)) { a.commenti += `${riga.trim()}\n`; return; }
    if (!riga.trim()) return;
    if (eVerso(riga)) {
    ...
    a.righeDiNote++;
    // Un commento in coda a una riga di note è un commento come gli altri: va nello stesso posto.
    const taglio = riga.indexOf("%");
    if (taglio >= 0) a.commenti += `${riga.slice(taglio).trim()}\n`;
    a.corpo += `${taglio >= 0 ? riga.slice(0, taglio) : riga}\n`;
...
// 15/09/2026 — «Non è in The Session: 393 risultati, nessuno che contenga «man», «in»».
// «in» è una preposizione: pretendere che compaia in un titolo è pretendere il nulla, e dirlo al
// Ghost come se fosse una spiegazione è peggio che tacere. Aggiunte le preposizioni delle due
// lingue che questo progetto usa davvero.
const PAROLE_IGNORATE_RILEVANZA = new Set([
  "the", "a", "an", "il", "lo", "la", "i", "gli", "le", "di", "del", "dei", "della", "e", "and", "of", "o", "or",
  "in", "on", "at", "to", "for", "from", "with", "by", "un", "una", "uno", "con", "per", "da", "su", "nel", "nella",
]);
...
export function spiegazioneRicerca({ query, strumento = "", autore = "", esito, archivio = ARCHIVIO_SPARTITI }) {
  ...
  // 15/09/2026, SECONDO GIRO, e stavolta il taglio è più netto. Il Ghost, con sei schermate
  // davanti: «ancora testo su testo di quasi spazzatura — io voglio lo spartito, non mi interessa
  // dove NON l'hai trovato (ancora con questo the session) — il resto è spreco di token e mio tempo
  // ed energie nel cercare la risposta vera nella risposta data».
  // Il giro precedente aveva accorciato la spiegazione. Non bastava, perché il difetto non era la
  // lunghezza: era che UN ESITO ZERO NON E' UNA NOTIZIA. Quando l'archivio non ha niente, la sua
  // riga deve stare in una riga sola e togliersi di mezzo — la risposta vera sta nella ricerca web,
  // che è quella sotto. Le note su strumento e autore sono roba da meccanico: restano SOLO quando
  // qualcosa è stato trovato, perché lì spiegano un risultato invece di allungare un fallimento.
  const note = [
  ...
  if (e.errore) {
    return { caso: "errore", altrove: [], testo: `${archivio.nome} non ha risposto: non è «non c'è», è «non ho potuto guardare».` };
  }
  // LE NOTE RESTANO ANCHE QUI, ed è l'unica cosa che ho rimesso dopo averla tolta. Dicono che ho
  // cercato una cosa DIVERSA da quella chiesta — «Antonio Vivaldi» tolto perché letto come autore —
  // e quello non è «dove non l'ho trovato»: è «potrei aver cercato la cosa sbagliata», che cambia
  // cosa fa il Ghost (corregge le parole nella casella, che è lì accanto). Un fatto si toglie solo
  // se togliendolo non si perde niente.
  if (!tunes.length && !grezzi) {
    return { caso: "nessuno", altrove, testo: coda(`Non è in ${archivio.nome} (lì c'è solo ${archivio.perChe}). Guardo nel resto del web.`) };
  }
  if (!tunes.length) {
    return { caso: "potati", altrove, testo: coda(`Non è in ${archivio.nome}: ${grezzi} ${grezzi === 1 ? "risultato" : "risultati"}, ${assenti.length ? `nessuno che contenga ${assenti.map((p) => `«${p}»`).join(", ")}` : "nessuno con tutte le parole che hai chiesto"}. Guardo nel resto del web.`) };
  }
...
// I REQUISITI CHE NASCONO DA QUELLO CHE E' STATO CHIESTO — 15/09/2026.
// Il brief dice «una linea di basso»: quella richiesta PORTA CON SE' un controllo, e il controllo
// si forma INSIEME alla decisione di chiedere, non a valle. È la regola di Anochin applicata al
// testo del Ghost invece che a una scheda di ricerca: l'accettore si specifica quando si agisce.
// L'ECCEZIONE E' ESPLICITA: se quello che si chiede E' una scala — un esercizio, un'arpeggio, uno
// studio — allora i gradi congiunti sono il risultato, non il difetto. Un controllo che non sa
// distinguere «scrivimi una scala» da «scrivimi una linea» boccerebbe la richiesta giusta.
const CHIEDE_UNA_LINEA = /\b(line[ae]|bassi?|melodi[ae]|tema|temi|riff|giro|groove|ostinato|walking)\b/i;
const CHIEDE_UNA_SCALA = /\b(scal[ae]|arpeggi?[oi]|esercizi?[oi]|studio|studi|modo|modi)\b/i;
export function requisitiDalBrief(brief) {
  const t = String(brief || "");
  if (!CHIEDE_UNA_LINEA.test(t) || CHIEDE_UNA_SCALA.test(t)) return [];
  return [{
    id: "non-e-una-scala",
    detta: "Una LINEA non è la scala della tonalità. Non scrivere le note del modo in fila per gradi: "
      + "scrivi una figura che torna, con salti alle note dell'accordo e un respiro ritmico. "
      + "La tonalità dice quali note puoi usare, non in che ordine metterle.",
    verifica: (a) => {
      const { corsa, note, quota } = corsaDiScala(a.corpo || a.abc);
      // Otto gradi congiunti di fila sono un'ottava intera: oltre quello non è una scaletta di
      // passaggio, è la scala. E la quota difende dal caso lungo: una corsa di otto dentro cento
      // note è musica, dentro dodici è tutto quello che c'è.
      return (corsa >= 8 || quota > 0.5)
        ? `sono ${corsa} note di fila per gradi congiunti su ${note}: è la scala della tonalità, non una linea`
        : null;
    },
  }];
}
...
// LO STRUMENTO IN INGLESE — 15/09/2026, dal Ghost: su «A Night in Tunisia» chiesto per BASSO, il
// pentagramma mostrato dalla ricerca diretta era per CLARINETTO. La parola «basso» non arrivava
// MAI alla ricerca — né alla query mandata a Serper né al filtro di pertinenza — quindi qualunque
// trascrizione del brano vinceva prima di quella giusta, e sullo schermo il titolo diceva
// letteralmente «CLARINET»: il web delle sheet music usa il nome dello strumento in inglese, non
// in italiano, e senza tradurlo la parola più discriminante della richiesta veniva ignorata.
const STRUMENTO_EN = {
  basso: "bass", bassi: "bass", contrabbasso: "double bass", contrabbassi: "double bass",
  chitarra: "guitar", chitarre: "guitar",
  violino: "violin", violini: "violin",
  viola: "viola", viole: "viola",
  violoncello: "cello", violoncelli: "cello",
  pianoforte: "piano", piano: "piano",
  flauto: "flute", flauti: "flute",
  ottavino: "piccolo", ottavini: "piccolo",
  clarinetto: "clarinet", clarinetti: "clarinet",
  oboe: "oboe",
  fagotto: "bassoon", fagotti: "bassoon",
  tromba: "trumpet", trombe: "trumpet",
  trombone: "trombone", tromboni: "trombone",
  corno: "horn", corni: "horn",
  sassofono: "saxophone", sassofoni: "saxophone", sax: "saxophone",
  mandolino: "mandolin", mandolini: "mandolin",
  fisarmonica: "accordion", fisarmoniche: "accordion",
  arpa: "harp", arpe: "harp",
  organo: "organ",
  batteria: "drums", batterie: "drums",
  voce: "voice", canto: "voice",
};
export function strumentoInInglese(strumento) {
  return STRUMENTO_EN[String(strumento || "").trim().toLowerCase()] || "";
}
...
export function briefRicercaWebSpartito({ query, autore = "", strumento = "" } = {}) {
  ...
  return [
    ...
    "",
    "Se in una pagina c'è l'IMMAGINE di uno spartito (una scansione, un'anteprima, un PNG o JPG),",
    "scrivi il suo indirizzo diretto così: IMMAGINE: https://…/qualcosa.png",
    "Dev'essere l'indirizzo del FILE immagine, non quello della pagina che la contiene. Il programma",
    "la fa leggere e la trascrive: è la strada che evita al Ghost di aprire i siti a uno a uno.",
    "Se sbagli l'indirizzo non succede niente di grave — il programma se ne accorge e lo dice.",
    "",
    "Se una pagina offre lo spartito in PDF, scrivi l'indirizzo del file così:",
    "PDF: https://…/qualcosa.pdf",
    "Dev'essere il file .pdf, non la pagina che lo offre. Anche questo il programma lo legge da sé,",
    "e conta più dell'immagine: un PDF è quasi sempre lo spartito INTERO, un'immagine spesso solo la",
    "prima pagina.",
    "SCRIVINE FINO A TRE, UNO PER RIGA, i gratuiti per primi — non scegliere tu il migliore.",
    "Un PDF su tre risponde vuoto o non si apre, e se ne hai dato uno solo il programma resta a mani",
    "vuote con altri cinque indirizzi buoni sullo schermo. E' successo davvero.",
    "",
...
// L'indirizzo dell'immagine di uno spartito, se la ricerca ne ha vista una. Si controlla che sia
// UN'IMMAGINE e non una pagina: passare a leggere una pagina HTML come se fosse un PNG produce un
// errore che non spiega niente. Se l'indirizzo è inventato il fallimento è rumoroso — il server
// risponde 404 e lo si dice — quindi qui basta la forma, non serve un guardiano più duro.
export function immagineDaRicerca(testo) {
  const dichiarata = (/^\s*IMMAGINE\s*:\s*(\S+)\s*$/im.exec(String(testo || "")) || [])[1] || "";
  const u = dichiarata.trim().replace(/[).,;]+$/, "");
  if (!/^https?:\/\//i.test(u)) return "";
  return /\.(png|jpe?g|gif|webp|bmp)(\?|$)/i.test(u) ? u : "";
}

// L'indirizzo di un PDF di spartito, se la ricerca ne ha visto uno. È il caso PIÙ FREQUENTE, non il
// più raro: nella ricerca vera del 15/09/2026 quasi tutti i risultati GRATIS erano PDF, non pagine.
// Parole del Ghost: «magari non trova il database ma trova un PDF o un immagine».
// Il PDF si legge per URL, MISURATO il 15/09/2026 con la chiave vera su un PDF vero di Mutopia
// («Mentre dormi amor», 131 KB): il server del modello lo scarica e lo legge per 0,0003 $, e il CORS
// del browser non c'entra — che qui è decisivo, perché Mutopia NON manda `access-control-allow-origin`
// (misurato: ASSENTE), quindi scaricarlo dall'app sarebbe stato impossibile.
// Attenzione a NON passarlo come immagine: OpenRouter rifiuta un PDF dentro `image_url`
// («Unsupported image format... Supported formats: PNG, JPEG, WebP, GIF»). Vuole la forma `file`.
export function pdfDaRicerca(testo) {
  const dichiarato = (/^\s*PDF\s*:\s*(\S+)\s*$/im.exec(String(testo || "")) || [])[1] || "";
  const u = dichiarato.trim().replace(/[).,;]+$/, "");
  if (!/^https?:\/\//i.test(u)) return "";
  return /\.pdf(\?|$)/i.test(u) ? u : "";
}

// TUTTI I DOCUMENTI DA PROVARE, NON IL PRIMO — 15/09/2026, dalla schermata del Ghost.
// La ricerca su «La Primavera» aveva trovato SEI posti gratuiti con un PDF. Il programma ne ha
// provato UNO — quello che il modello aveva dichiarato, englishtap.com — che ha risposto con un
// corpo vuoto, e si è fermato lì dicendo «preferisco non darti uno spartito rotto», mentre a due
// centimetri sullo schermo c'erano flutetunes, free-scores, IMSLP e Mutopia.
// Un tentativo su sei non è un tentativo: è una scusa. Qui si mettono in fila tutti i candidati,
// quelli dichiarati dal modello E quelli che stanno negli indirizzi VERI della ricerca.
//
// L'ordine è quello di uno che ha fretta: prima GRATIS (chi vende dà l'anteprima, non lo spartito),
// prima i PDF delle immagini (un PDF è quasi sempre il pezzo intero), e a parità l'ordine in cui la
// ricerca li ha restituiti.
export function candidatiDaLeggere({ testo = "", fonti = [], max = 4 } = {}) {
  const pulisci = (u) => String(u || "").trim().replace(/[).,;]+$/, "");
  const fuori = [];
  const visti = new Set();
  const aggiungi = (url, gratis) => {
    const u = pulisci(url);
    if (!/^https?:\/\//i.test(u) || visti.has(u)) return;
    const ePdf = /\.pdf(\?|$)/i.test(u);
    if (!ePdf && !/\.(png|jpe?g|gif|webp|bmp)(\?|$)/i.test(u)) return;
    visti.add(u);
    fuori.push({ url: u, pdf: ePdf, che: ePdf ? "il PDF" : "l'immagine", gratis: !!gratis });
  };
  // 1 · Quelli che il modello ha DICHIARATO: ha letto le pagine, sa dove sta il file.
  //     Più d'uno, se ne ha scritti più d'uno — il brief adesso lo chiede.
  for (const m of String(testo || "").matchAll(/^\s*(?:PDF|IMMAGINE)\s*:\s*(\S+)\s*$/gim)) aggiungi(m[1], false);
  // 2 · Quelli che stanno negli INDIRIZZI VERI restituiti dal motore. Il modello può non averli
  //     nominati, ma il programma ce li ha in mano: un .pdf fra i risultati è un candidato.
  for (const f of Array.isArray(fonti) ? fonti : []) aggiungi(f?.url, f?.costo === "gratis");
  return fuori
    .map((c, i) => ({ ...c, i }))
    .sort((a, b) => (Number(b.gratis) - Number(a.gratis)) || (Number(b.pdf) - Number(a.pdf)) || (a.i - b.i))
    .slice(0, Math.max(1, max))
    .map(({ i, ...c }) => c);
}

// ARRIVARE DIRETTAMENTE ALL'IMMAGINE — 15/09/2026, dal Ghost dopo aver misurato la differenza:
// «Englishman in New York» per clarinetto, quindici secondi a mano — Google Immagini, MuseScore,
// il pentagramma sott'occhio. La catena dell'app trovava lo stesso indirizzo (`candidatiDaLeggere`
// lo mette in fila per la LETTURA) ma non lo mostrava mai: `webImmagine`/`webPdf` finivano
// calcolati e salvati nello stato e nessuna schermata li leggeva — lo stesso difetto di un valore
// di ritorno che nessuno guarda, qui su un dato invece che su un booleano.
// Serve LA MOSSA DEL GHOST, non un sostituto: vedere il pentagramma prima e indipendentemente
// dalla trascrizione, che è più lenta (passa da un modello) e può fallire. Un PDF non si disegna
// con un tag <img> — servirebbe un lettore che non c'è — quindi qui si prende il primo candidato
// che è davvero un file immagine, scartando i PDF anche se vengono prima in `candidatiDaLeggere`.
export function primaImmagineMostrabile(candidati) {
  return (Array.isArray(candidati) ? candidati : []).find((c) => c && !c.pdf)?.url || "";
}

// Il numero di catalogo che il modello ha trovato: serve al programma per comporre le ricerche a
...
export function titoliDaRicerca(testo) {
  // UN CAMPO VUOTO NON DEVE MANGIARSI LA RIGA DOPO. Misurato su una ricerca vera il 15/09/2026:
  // il modello ha lasciato TITOLO_ZH vuoto e nel campo è finito «- flutetunes.com — pentagramma,
  // PDF, MIDI, MP3 — gratis», cioè la prima riga dell'elenco dei risultati. Un titolo è CORTO e non
  // contiene i segni di un elenco o di un dominio: se li contiene non è un titolo, è altro.
  const nonEUnTitolo = (v) => !v
    || /^(?:-|–|—|nessuno|non trovato|vuoto|n\.?a\.?|null)$/i.test(v)
    || /^[-–—•*]\s/.test(v)                       // comincia come una voce di elenco
    || /\.(com|org|net|it|io)\b/i.test(v)         // c'è dentro un dominio
    || /\s[—–]\s/.test(v)                         // c'è un trattino lungo da elenco
    || v.length > 60;
  const campo = (nome) => {
    const v = (new RegExp(`^${nome}\\s*:[ \\t]*(.*)$`, "im").exec(String(testo || "")) || [])[1]?.trim() || "";
    return nonEUnTitolo(v) ? "" : v.slice(0, 60);
  };
  return { en: campo("TITOLO_EN"), zh: campo("TITOLO_ZH") };
}
// UN CATALOGO CHE NON E' UN CATALOGO AVVELENA OGNI RICERCA A VALLE — 15/09/2026, misurato sullo
// schermo del Ghost. Su «Englishman in New York» la card diceva: «il numero di catalogo è
// TITOLO_EN: Englishman in New Y», e quel testo finiva DENTRO le ricerche già pronte. Toccandone
// una, Google riceveva `TITOLO_EN: Englishman in New Y Sting spartito filetype:pdf -site:...` e
// rispondeva «non ha prodotto risultati in nessun documento». Sei link su sei, tutti morti.
// Il meccanismo è lo stesso già trovato nei titoli: il modello lascia CATALOGO vuoto, e il campo si
// mangia la riga dopo — che è l'etichetta successiva. Là l'avevo corretto, qui no, perché avevo
// guardato il sintomo e non la forma. Un numero di catalogo HA UNA FORMA (RV 269, BWV 1048, K. 550,
// Op. 8 No. 1, D. 960): se quello che c'è scritto non ce l'ha, non è un numero di catalogo.
const FORMA_CATALOGO = /^(?:RV|BWV|BW|K|KV|D|Op|Hob|WoO|L|S|TWV|HWV|Anh)\b[\s.]*\d/i;
export function catalogoDaRicerca(testo) {
  const dichiarato = (/^\s*CATALOGO\s*:\s*(.+)$/im.exec(String(testo || "")) || [])[1];
  if (dichiarato && FORMA_CATALOGO.test(dichiarato.trim())) return dichiarato.trim().slice(0, 30);
...
// PRIMA CHE PARLI DEL BRANO GIUSTO, POI TUTTO IL RESTO — 15/09/2026, dalla schermata.
// Cercando «Englishman in New York di Sting», i primi cinque risultati erano: «Sheet Music Shop»
// (due volte), «John Dowland Sheet Music», «Joplin, Scott - The Entertainer» (due volte). Tutti
// marcati GRATIS, tutti in cima, nessuno che c'entrasse qualcosa — mentre le tre trascrizioni vere
// di Englishman in New York stavano SOTTO, perché ScoreExchange non è marcato gratis.
// Il difetto era mio e l'avevo già trovato una volta: è la stessa forma della ricerca in OR
// dell'archivio, dove cento brani irlandesi rispondevano a «One dei Metallica». Lì un filtro c'era
// (`risultatiCheRispondono`); qui no, e l'ordinamento metteva il COSTO davanti a tutto.
// Un risultato gratis sul brano sbagliato non è un risultato: è rumore con una medaglia.
const pertinenzaFonte = (parole, f) => {
  if (!parole.length) return 1;
  const pagliaio = normalizzaPerConfronto(`${f?.titolo || ""} ${decodeURIComponent(String(f?.url || ""))}`);
  return parole.filter((p) => pagliaio.includes(p)).length / parole.length;
};
export function fontiPerSpartito(fonti, query = "") {
  ...
  return lista
    ...
    .map((f) => {
      ...
      return { ...f, che: noto ? noto.che : "", portabile: noto ? noto.portabile : "", costo: noto ? noto.costo : "", pertinenza: pertinenzaFonte(parole, f) };
    })
    // ZERO PAROLE IN COMUNE = ALTRO BRANO. Non si mostra, non si ordina: si toglie. Con una query
    // vuota (nessuna parola utile) la pertinenza vale 1 per tutti e non si scarta niente — non si
    // afferma l'irrilevanza a partire da un confronto che non è stato fatto.
    .filter((f) => f.pertinenza > 0)
    // PERTINENZA PRIMA DI TUTTO. Poi, a parità, il costo: fra due trascrizioni dello stesso brano
    // quella gratis vince. Prima era il contrario, ed è per questo che Joplin stava in cima.
    .sort((a, b) => (b.pertinenza - a.pertinenza)
      || (pesoCosto[a.costo] - pesoCosto[b.costo])
      || (pesoForma[a.portabile] - pesoForma[b.portabile]));
}

// LA QUERY CHE SI MANDA E QUELLA CHE SI GIUDICA DEVONO ESSERE LA STESSA — 15/09/2026, dal Ghost,
// due volte: «basso» per «A Night in Tunisia» ha dato il CLARINETTO, «violoncello» per «La
// Cavalcata delle Valchirie» ha dato una riduzione per PIANOFORTE. In tutti e due i casi lo
// strumento chiesto non arrivava da nessuna parte: non nella query a Serper, non nel filtro di
// pertinenza. Un oggetto solo, una funzione sola, usata due volte (stessa idea di
// `detta`/`verifica` nella stessa riga di `lib/capitolato.js`): comporle in due punti diversi è
// il modo garantito per dimenticare lo stesso pezzo in uno dei due, com'è successo qui.
export function queryImmagineDiretta({ query = "", autore = "", strumento = "" } = {}) {
  const en = strumentoInInglese(strumento);
  return [String(query || "").trim(), String(autore || "").trim(), en, "sheet music"].filter(Boolean).join(" ").trim();
}

// ══════════════════════════════════════════════════════════════════════════════
// LA RICERCA DIRETTA PER IMMAGINI — 15/09/2026, da una domanda del Ghost.
// ══════════════════════════════════════════════════════════════════════════════
// «Non le abbiamo già le API per la ricerca online, ad esempio quella di Balthasar?» — no: quella
// è `openrouter:web_search`, un MODELLO che cerca e RACCONTA cosa ha trovato in prosa, da cui
// `candidatiDaLeggere`/`immagineDaRicerca` estraggono indirizzi con una regex. Qui invece i
// risultati arrivano da un'API di ricerca immagini vera (Serper — vedi `api/cerca-immagine-spartito.js`):
// oggetti strutturati con un indirizzo di file reale, non testo da interpretare. Stesso identico
// filtro di pertinenza di `fontiPerSpartito`, riusato e non riscritto — due copie divergerebbero
// entro un mese, ed è già successo col piano alimentare.
export function immaginiSpartitoDaSerper(grezzi, query = "") {
  const parole = normalizzaPerConfronto(query).split(" ").filter((p) => p && !PAROLE_IGNORATE_RILEVANZA.has(p));
  return (Array.isArray(grezzi) ? grezzi : [])
    .filter((im) => im && /^https?:\/\//i.test(String(im.imageUrl || "")))
    .map((im) => ({
      url: String(im.imageUrl).trim(),
      pagina: String(im.link || "").trim(),
      titolo: String(im.title || "").trim(),
      dominio: dominioNudo(im.domain || ""),
      pertinenza: pertinenzaFonte(parole, { titolo: im.title, url: im.link || im.imageUrl }),
    }))
    // STESSA REGOLA DI `fontiPerSpartito`: zero parole in comune con la richiesta è un altro
    // brano, non uno "zero" da mostrare comunque. Con query vuota `pertinenzaFonte` vale 1 per
    // tutti e non scarta niente.
    .filter((im) => im.pertinenza > 0)
    .sort((a, b) => b.pertinenza - a.pertinenza);
}
...
export function briefDiTrascrizione({ strumento = "", conVersi = false } = {}) {
  ...
  return [
    ...
    "",
    "NON INVENTARE. Non stai componendo: stai copiando. Se UNA battuta non si legge, NON riempirla a",
    "orecchio: scrivila come pausa e dillo in una riga di commento che comincia con %. Una battuta",
    "inventata è peggio di una mancante, perché non si distingue da una letta bene.",
    "",
    "MA SE NON RIESCI A VEDERE IL DOCUMENTO — è vuoto, è illeggibile, non è uno spartito — allora",
    "NON PRODURRE ABC. Scrivi una riga sola che comincia con % e dice cosa è successo, e basta.",
    "Non scrivere un'intestazione seguita da pause: una pagina di pause ha la forma di uno spartito e",
    "il programma non ha modo di distinguerla da un brano che comincia in silenzio.",
    "È già successo (15/09/2026): quattro battute di pause sono finite sullo schermo del Ghost come",
    "«spartito trovato», col pulsante per tenerlo. Meglio niente che una cosa che sembra musica.",
    "",
    "TRASCRIVI TUTTO QUELLO CHE VEDI, non solo l'inizio. Se il documento ha quattro pagine, la",
    "trascrizione non finisce alla prima riga. E se sei riuscito a leggerne solo una parte, DILLO in",
    "un commento % dicendo QUALE parte (per esempio: «% lette le prime 16 battute, la pagina 2 non si",
    "apre»): chi legge deve sapere se ha in mano il brano o un pezzo.",
    "",
...
export function richiestaDiSpartito(frase) {
  ...
  // SE LA FRASE NOMINA DUE SINONIMI («la TABLATURA e lo SPARTITO per basso di Red dei King
  // Crimson»), l'ancora sopra prende il PRIMO — è un `.exec` senza `/g` — e il secondo resta
  // appiccicato in testa al resto come rumore: «e lo spartito di Red...». Trovato dal Ghost: la
  // query finiva mostrata come «e spartito Red», con King Crimson perso per strada (l'autore lo
  // becca comunque CODA_AUTORE più sotto, ma il titolo usciva rotto). Si toglie SOLO in testa e
  // SOLO un secondo oggetto — non un titolo che comincia per coincidenza con «e» o «o».
  resto = resto.replace(new RegExp(`^\\s*(?:e|o)\\s+(?:l[oa]\\s+)?${OGGETTO_SPARTITO.source}\\s*`, "i"), "");
