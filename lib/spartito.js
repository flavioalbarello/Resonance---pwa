...
export function queryScaltre({ query, autore = "", catalogo = "" } = {}) {
  const q = String(query || "").trim();
  const chi = String(autore || "").trim();
  const cat = String(catalogo || "").trim();
  const meno = NEGOZI_DA_ESCLUDERE.map((d) => `-site:${d}`).join(" ");
  const fuori = [];
  if (cat) fuori.push(`"${cat}" ${chi} spartito filetype:pdf`);      // il numero di catalogo apre tutto
  fuori.push(`${q} ${chi} site:imslp.org`.trim());
  fuori.push(`${q} ${chi} spartito filetype:pdf ${meno}`.trim());
  fuori.push(`${q} ${chi} sheet music free download ${meno}`.trim());
  return fuori.filter(Boolean);
}
// Le ricerche che il Ghost può fare LUI in un tocco, già composte. La ricerca per IMMAGINI è la
// più utile di tutte per uno spartito e nessun modello la sa fare: mostra il pentagramma prima di
// aprire qualunque pagina.
export function ricercheAMano({ query, autore = "", catalogo = "" } = {}) {
  const base = [query, autore].filter(Boolean).join(" ").trim();
  const conCat = [catalogo || query, autore].filter(Boolean).join(" ").trim();
  const g = (s) => encodeURIComponent(s);
  return [
    { id: "immagini", nome: "Google Immagini", che: "vedi il pentagramma senza aprire niente — la mossa più veloce per uno spartito",
    ...
    { id: "pdf", nome: "Solo PDF, senza negozi", che: "va al documento invece che alla pagina di vendita",
      url: `https://www.google.com/search?q=${g(`${conCat} spartito filetype:pdf ${NEGOZI_DA_ESCLUDERE.map((d) => `-site:${d}`).join(" ")}`)}` },
...
export function briefRicercaWebSpartito({ query, autore = "", strumento = "" } = {}) {
  ...
  return [
    ...
    "· prova anche il titolo nelle ALTRE LINGUE: «La Primavera» è anche «Spring», «Frühling»,",
    "  «Le Printemps» — la versione libera può stare sotto un altro nome.",
    "· se il primo giro dà solo negozi, CAMBIA CHIAVE invece di insistere.",
    "",
    "Se hai trovato il numero di catalogo scrivilo come prima riga così: CATALOGO: RV 269",
